	...
	onMount(async () => {
		try {
			...
			if (res.ok) {
				...
				if (data.callsign) callsign = data.callsign;
			}
		} catch {
...
<svelte:head>
	<title>About — goMeshCom</title>
</svelte:head>

<main class="min-h-screen bg-[#111827] px-4 py-10 text-gray-100">
	<div class="mx-auto max-w-xl space-y-8">
		<!-- back -->
		<a href="{base}/" class="text-sm text-gray-500 hover:text-gray-300">← Back</a>
		...
		<div class="rounded-md border border-gray-700/40 bg-[#1a2030] p-5">
			...
			<p class="mt-2 text-xs leading-relaxed text-gray-500">
				...
				any damage arising from its use. Use of this software for radio communications is subject to
				the regulations of your national telecommunications authority.
			</p>
		</div>
	</div>
</main>
