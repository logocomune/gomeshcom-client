...
Rendere l'interfaccia fruibile su smartphone (portrait, ~360×740). Breakpoint: `md` (768px). Sotto `md` = mobile layout; sopra = desktop invariato.
...
| Viewport | Atteso |
|---|---|
| 360×740 (phone) | Header: logo + callsign + mini dot. Pannelli impilati: Chat → Map → UDP. No drag handles. Campi UDP ridotti. Tap riga → modale JSON. |
| 768×1024 (tablet) | Layout desktop completo |
| 1280×800 (desktop) | Identico a prima |
